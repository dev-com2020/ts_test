...
When('użytkownik wybiera miejsce docelowe {string}', function (destination) {
    // Implementacja kroku
    console.log(`Użytkownik wybiera miejsce docelowe: ${destination}`);
});
When('użytkownik wybiera datę podróży {string}', function (date) {
    // Implementacja kroku
...
Given('użytkownik wybrał lot {string} do {string}', function (flight, destination) {
    // Implementacja kroku
...
When('użytkownik dokonuje płatności kartą {string}', function (cardNumber) {
    // Implementacja kroku
